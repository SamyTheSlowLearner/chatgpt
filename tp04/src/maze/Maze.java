...
public class Maze implements Graph{
	...
	public ArrayList<Vertex> getSuccesseurs(Vertex a) {
	    ...
	    int x = box.getabs();
	    int y = box.getord();
	    // Check the six neighboring hexagons
	    try {
	        if (Cases[x+1][y].passante()==1) {
	            successors.add(Cases[x+1][y]);
	        }
	    } catch (Exception e) {}
	    try {
	        if (Cases[x-1][y].passante()==1) {
	    ...
	    try {
	        if (Cases[x][y+1].passante()==1) {
	            successors.add(Cases[x][y+1]);
	        }
	    } catch (Exception e) {}
	    try {
	        if (Cases[x][y-1].passante()==1) {
	    ...
	    try {
	        if (x%2==0) {
	            if (Cases[x-1][y+1].passante()==1) {
	                successors.add(Cases[x-1][y+1]);
	            }
	        } else {
	            if (Cases[x+1][y+1].passante()==1) {
	                successors.add(Cases[x+1][y+1]);
	...
	{			  
		...
		this.hauteur=hauteur(fileName);
		this.largeur=largeur(fileName);
		this.Cases = new MazeBox[this.hauteur][this.largeur];
		try {
	...
	public MazeBox depart() throws Exception  {
		ArrayList<MazeBox> test1 = new ArrayList<MazeBox>();
		this.saveToTextFile("data/expected-result");
		for(int i =0;i<hauteur("data/expected-result");i++) {
			for(int j=0;j<largeur("data/expected-result");j++) {
	...
	public void update( ArrayList<Vertex> chemin) throws Exception {
